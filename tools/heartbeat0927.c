...
void BroadcastHeartbeat(uchar_t hb_slot_count, char *slot_list,char status)
{
 //uchar_t uc=0,ut=0;
 //char *values=NULL;
 ...
 else
 {
  ...
  printf("BroadcastHeartbeat status=%d hb_slot_count=%d slot_list:",status,hb_slot_count);
   ...
   printf("\n");
 */
 ioBroadcast(0,hb_slot_count,slot_list,message,3);
...
void check_heart_beat()
{
  ...
  uchar_t uc=0,ut=0;
  char status;
  uchar_t hb_start_slot_count=0,hb_stop_slot_count=0;
  char hb_start_slot_list[16],hb_stop_slot_list[16];
  short alarm_id[10]={0};
  uchar_t alarm_count=0;
  time_t alarm_time[10];
  time_t now_time;
  ...
  for (n=0;n<hb_slot_count;n++)
  {
     ...
     // printf("slot=%d hb_source[n].recieved_ips[m]=%d\r\n",hb_source[n].slot,hb_source[n].recieved_ips[m]);
      if (hb_source[n].recieved_ips[m]!=1)
      {
         if (hb_source[n].status!=2)
         {
           ...
           hb_stop_slot_list[hb_stop_slot_count]=hb_source[n].slot;
           hb_stop_slot_count++;
           ...
           if (heartbeat_state!=2)
           {
                if (heartbeat_state==1)
                {
                  ...
                  //sendAlarm(19,0,getSlotOfNmu(0,2),0,0,0,0,0);
                  if (alarm_count<10)
                  {
                     alarm_id[alarm_count]=19;
                     time(&now_time);
                     alarm_time[alarm_count]=now_time;
                     //time(alarm_time[alarm_count]);
                     alarm_count++;
                  }
                  if (invalid_message)
                  {
                    //sendAlarm(15,0,getSlotOfNmu(0,2),0,0,0,0,0);
                    if (alarm_count<10)
                    {
                     alarm_id[alarm_count]=15;
                     time(&now_time);
                     alarm_time[alarm_count]=now_time;
                     alarm_count++;
                    }
    ...
    if (status)
    {
       if (hb_source[n].status!=1)
       {
          ...
          hb_start_slot_list[hb_stop_slot_count]=hb_source[n].slot;
          hb_start_slot_count++;
          if (heartbeat_state!=1)
          {
              if (heartbeat_state==2)
              {
                if (alarm_count<10)
                {
                   alarm_id[alarm_count]=20;
                   time(&now_time);
                   alarm_time[alarm_count]=now_time;
                   alarm_count++;
                }
                //sendAlarm(20,0,getSlotOfNmu(0,2),0,0,0,0,0);
              }
              heartbeat_state=1;
          }
  ...
  if (hb_start_slot_count>0)
  {
    //printf("Heartbeat start_slot_count=%d\n",hb_start_slot_count);
    BroadcastHeartbeat(hb_start_slot_count,hb_start_slot_list,1);
  }
  if (hb_stop_slot_count>0)
  {
   //printf("Heartbeat stop_slot_count=%d\n",hb_stop_slot_count);
   BroadcastHeartbeat(hb_stop_slot_count,hb_stop_slot_list,0);
  }
  for (n=0;n<alarm_count;n++)
  {
    //printf("alarm_time[%d]=%d\n",n,alarm_time[n]);
    sendAlarm(alarm_id[n],0,getSlotOfNmu(0,2),0,0,0,0,alarm_time[n]);
  }
  for (n=0;n<hb_slot_count;n++)
  {
    for (m=0;m<hb_source[n].ip_count;m++)
...
/*
void BroadcastHeartbeat(uchar_t slot_count, char *slot_list,char status)
{
 uchar_t uc=0,ut=0;
 char *values=NULL;
 ...
 ioBroadcast(0,slot_count,slot_list,message);
