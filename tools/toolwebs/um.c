...
#define UM_DB_FILENAME	T("/vispace/webs/um.xml")
...
#define UM_LEVEL		T("level")
#define UM_METHOD		T("method")
#define UM_LEVELILEGE	T("priv")
 ...
 */
#define NUMBER_OF_USER_COLUMNS	6

char_t	*userColumnNames[NUMBER_OF_USER_COLUMNS] = {
			UM_NAME, UM_PASS, UM_GROUP, UM_PROT, UM_DISABLE,UM_LEVEL
};

int		userColumnTypes[NUMBER_OF_USER_COLUMNS] = {
			T_STRING, T_STRING, T_STRING, T_INT, T_INT, T_INT
};
...
char_t	*groupColumnNames[NUMBER_OF_GROUP_COLUMNS] = {
			UM_NAME, UM_LEVELILEGE, UM_METHOD, UM_PROT, UM_DISABLE,UM_LEVEL
};

int		groupColumnTypes[NUMBER_OF_GROUP_COLUMNS] = {
			T_STRING, T_INT, T_INT, T_INT, T_INT,T_INT
};
...
int	umCommit(char_t *filename)
{
	...
	trace(3, T("UM: Writing User Configuration to file <%s>\n"), 
		saveFilename);
        //printf("UM: Writing User Configuration to file <%s>\n", \
		saveFilename);
 ...
 *	column data.
 */
        //printf("tableName:%s, columnName:%s\n",tableName,columnName);
	columnData = NULL;
...
/*******************************************************************************/
/*
 *	umGetUserPermission() returns the user Permission
 */
/*
short umGetUserPermission(char_t *user)
{

 int perm, row;
 char_t * group=umGetUserGroup(char_t * user);
 if (NULL==group)
    return -1;
 a_assert(group && *group);
 perm = -1;
 row = dbSearchStr(didUM, UM_GROUP_TABLENAME, UM_GROUP, user, 0);

 if (row >= 0) {
	dbReadInt(didUM, UM_USER_TABLENAME, UM_LEVEL, row, &perm);
 }

 return (short) perm;
}
*/
short umGetUserPrivilege(char_t *user)
{
  short privilege;
  char_t *ug=umGetUserGroup(user);
  if (ug!=NULL)
  {
...
/******************************************************************************/
/*
 *	umGetUserLevel() returns the user level
 */

short umGetUserLevel(char_t *user)
{
 ...
 if (row >= 0) {
	dbReadInt(didUM, UM_USER_TABLENAME, UM_LEVEL, row, &userlevel);
 }

 return (short) userlevel;
}

int umSetUserLevel(char_t *user, int level)
{
  int row;
  if (level<1 || level>2)
     return UM_ERR_GENERAL;
 ...
 */
	row = dbSearchStr(didUM, UM_USER_TABLENAME, UM_NAME, user, 0);
	if (row >= 0) {
		return dbWriteInt(didUM, UM_USER_TABLENAME, UM_LEVEL, row, level);
	} else {
		return UM_ERR_NOT_FOUND;
	}
}
 ...
 */
	dbWriteInt(didUM, UM_GROUP_TABLENAME, UM_LEVELILEGE, row, priv);
...
short umGetGroupPrivilege(char_t *group)
{
	...
	if (row >= 0) {
		dbReadInt(didUM, UM_GROUP_TABLENAME, UM_LEVELILEGE, row, &privilege);
	}
...
int	umSetGroupPrivilege(char_t *group, short privilege)
{
	...
	if (row >= 0) {
		return dbWriteInt(didUM, UM_GROUP_TABLENAME, UM_LEVELILEGE, row, 
			(int)privilege);
	} else {
		return UM_ERR_NOT_FOUND;
	}
