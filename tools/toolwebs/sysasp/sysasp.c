...
static int aspKeepAliveDestHostList(int eid, webs_t wp, int argc, char_t **argv)
{
  ...
  while (ret==SQLITE_ROW)
  {
   //nbytes+=websWrite(wp,T("item[%d]= new Object();item[%d].id=%d;item[%d].ip=%s;item[%d].port=%d;item[%d].ena=%d;item[%d].hname=%s;item[%d].val=%d;\n"),n,n,sqlite3_column_int(stmt,0),n,sqlite3_column_text(stmt,1),n,sqlite3_column_int(stmt,2),n,sqlite3_column_int(stmt,3),n,sqlite3_column_text(stmt,4),n,sqlite3_column_int(stmt,5));
...
static void formDestHost(webs_t wp, char_t *path, char_t *query)
{
  ...
  else  if (!gstrcmp(operate,"add"))
  {
     ...
     /*if (gstrcmp(target_name,""))
     {
      sprintf(sql,"SELECT count(*) FROM DestHost WHERE target_name='%s';",target_name);
      if (getRowCount(NULL,sql)<1)
      {
      sprintf(sql,"INSERT INTO \"DestHost\" VALUES(NULL,'%s',%s,%d,'%s');",destIp,udp, enable,target_name);
      execSql(NULL,sql,NULL,NULL);
      insertOperateLog(LOG_GROUP_SYS_SNMP_DEST_HOST_CONF,0,0,WebMode,add,0,0);
      }
     }
