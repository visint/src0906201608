...
static int aspAlarmTypeList(int eid, webs_t wp, int argc, char_t **argv)
{
  ...
  char sql[120]="SELECT alarm_code,alarm_name,entity_name,severity,enable,mask_slot,is_save FROM AlarmType WHERE usable<>2";
  //char severity[6][16]={"Unkown","紧急告警", "严重告警","主要告警" ,"次要告警" ,"提示告警"};
  char severity[6][16]={"Unkown","紧急告警", "主要告警","次要告警" ,"提示告警" ,"清除告警"};
  #else
  char sql[160]="SELECT alarm_code,alarm_name_en as alarm_name,entity_name_en as entity_name,severity,enable,mask_slot,is_save FROM AlarmType WHERE usable<>2";
  ...
  //if (strcmp(column_value[0],"0"))
    ...
    if (1!=sqlite3_column_int(stmt,6))
     ...
     nBytes+=websWrite((webs_t)wp,T("<td>Yes</td>"));
  #endif
    nBytes+=websWrite((webs_t)wp,T("<td>%s</td>"),sqlite3_column_text(stmt,5));
...
static int aspAlarmType(int eid, webs_t wp, int argc, char_t **argv)
{
  ...
  //printf("aspOperateLog2 sq:%s DB_FILE_NAME=%s\n",sql,DB_FILE_NAME);
    #if DEFAULT_LANG_CN
     sprintf(sql,"SELECT alarm_code,alarm_name,severity,enable,mask_slot,is_save FROM AlarmType WHERE alarm_code=%d LIMIT 1",alarmcode);
    #else
     sprintf(sql,"SELECT alarm_code,alarm_name_en,severity,enable,mask_slot,is_save FROM AlarmType WHERE alarm_code=%d LIMIT 1",alarmcode);
    #endif
    ret=sqlite3_prepare_v2(db,sql,strlen(sql),&stmt,NULL);
    if(ret!=SQLITE_OK)
    {
...
static void formAlarmType(webs_t wp, char_t *path, char_t *query)
{
  ...
  if (!gstrcmp(operate,"0"))
  {
     ...
     #ifdef DEFAULT_LANG_CN
        sprintf(sql,"SELECT alarm_code,alarm_name,severity,enable,mask_slot FROM AlarmType WHERE alarm_code=%s",alarm_code);
     #else
        sprintf(sql,"SELECT alarm_code,alarm_name_en,severity,enable,mask_slot FROM AlarmType WHERE alarm_code= FROM AlarmType WHERE alarm_code=%s",alarm_code);
     #endif
  ...
  else if (!gstrcmp(operate,"mod"))
  {
      ...
      if (sid>0 && sid <6)
      {
        sprintf(sql,"UPDATE AlarmType SET enable=%d,is_save=%d,severity=%s,mask_slot='%s' WHERE alarm_code=%s",ena,is_save,severity,slots,alarm_code);
        //printf("sql:%s\n",sql);
