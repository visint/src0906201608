...
function check(obj,item,port)
{
  ...
  if (item>0 && item<11)
  {
    ...
    switch (item){
     case 1:
     case 3:
          ret=checkNumber(obj,-70,0,msg +n1);
          break;
     case 2:
     case 4:
           ret=checkNumber(obj,-30,5,msg +n2);
          break;
     case 5:
          ret=checkNumber(obj,-30,20,msg +n1);
          break;
     case 6:
     ...
     case 8:
          ...
          break;
     case 9:
          ret=checkNumber(obj,1,30,msg +n1);
          break;
     case 10:
          ret=checkNumber(obj,10,150,msg +n2);
          break;
    }
    if (ret!=true)
      return false;
...
function showDdmtTable()
{
	if (null==rxMin)
	  return;
	var i;
	for (i=0;i<rxMin.length;i++)
	{
		if (rxMin[i]!=null)
		{
		dw(F("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td><td>{6}</td><td>{7}</td><td>{8}</td><td>{9}</td><td>{10}</td>",i+1,rxMin[i],rxMax[i],txMin[i],txMax[i],tMin[i],tMax[i],vMin[i],vMax[i],bMin[i],bMax[i]));
		if (ut==1)
		dw("<td></td>");
		dw("</tr>");
	 }
...
<table width="100%" align="center" cellpadding="0" cellspacing="0">
 <tr>
  ...
  showDdmtTable();
   ...
   <tr>
   <td colspan="12" align="center"><a class="button white medium" href="otuddmt.asp">刷新</a></td>
  </tr>
   </table>   
   </td>
  </tr>
