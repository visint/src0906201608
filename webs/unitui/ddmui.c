...
static int aspDdmInfo(int eid, webs_t wp, int argc, char_t **argv)
{
  ...
  nBytesSent+=websWrite(wp, T("var slot=%d,uc=%d,ut=%d;\n"),slot,uc,utype);
  if (utype==1)
    port_count=4;
  else  if(utype==OTU_TYPE_10G_SFP_PLUS_A)
  {
    ...
    port_count=8;
  //printf("port_count=%d\n",port_count);
  uchar_t sft_exist[8]={0};
  uchar_t editable[8]={0};
  char *values;
  ...
  if (utype<15)
  {
   ...
   {  
    ...
    for (n=0;n<8;n++)
     sft_exist[n]=pOtuStatus->portStatus[n].on_module;
   }
  }
  else if (NULL!=values)
  {
     char bits[8];
     otu1XInfo_t *otu1x=(otu1XInfo_t*)values;
     bytetobits(otu1x->exist & 0xff,bits);
     for (n=0;n<8;n++)
     {
       sft_exist[n]=bits[7-n];
       editable[n]=(otu1x->src_thld>>n) & 0x1;
       /*
       if (bits[7-n]==1)
          sft_exist[n]=1;
       else
          sft_exist[n]=2;
       */
       //printf("#%d sft_exist[n]=%d bits[7-n] %d",n+1,sft_exist[n],bits[7-n]);
     }
  }
  if (!gstrcmp(group,"1"))
  {
...
*/
 ...
 {
   ddmThreshold_t *ddmt;
   nBytesSent+=websWrite(wp,T("var editable=new Array(%d),rxMin=new Array(%d),rxMax=new Array(%d),txMin=new Array(%d),txMax=new Array(%d),tMin=new Array(%d),tMax=new Array(%d),vMin=new Array(%d),vMax=new Array(%d),bMin=new Array(%d),bMax=new Array(%d);\n"),port_count,port_count,port_count,port_count,port_count,port_count,port_count,port_count,port_count,port_count,port_count);
   ...
   for (n=1;n<port_count+1;n++)
   {
     ...
     if (utype!=2)
     {
       ...
       if (1==sft_exist[n-1])
       {
        nBytesSent+=websWrite(wp, T("editable[%d]=%d;\n"),n-1,editable[n-1]);
 ...
 else if (!gstrcmp(group,"3"))
 {
    n=getSelectedPort();
    ddmThreshold_t *ddmt=getPortDdmThreshold(0, slot,n);
    if (ddmt!=NULL)
    {
      nBytesSent+=websWrite(wp, T("rxMin=%d;rxMax=%d;"),ddmt->rxPowerMin,ddmt->rxPowerMax);
       nBytesSent+=websWrite(wp, T("txMin=%d;txMax=%d;"),ddmt->txPowerMin,ddmt->txPowerMax);
       nBytesSent+=websWrite(wp, T("tMin=%d;tMax=%d;"),ddmt->tempMin,ddmt->tempMax);
       nBytesSent+=websWrite(wp, T("vMin=%d;vMax=%d;"),ddmt->voltMin,ddmt->voltMax);
       nBytesSent+=websWrite(wp, T("bMin=%d;bMax=%d;\n"),ddmt->rxBiasMin,ddmt->rxBiasMax);
    }
 }
 else
 {  
