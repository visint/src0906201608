...
int asp4UNmuView(webs_t wp,uchar_t utype,uchar_t slot)
{
    chassis_t *pChassis;
    char fan=2;
    char sfp1=2,sfp2=2,sfp3=2,sfp1_led=2,sfp2_led,run_led=1,lan[2]= {2},power[2]= {2};
    int nBytesSent=0;
    char led_type='\0';
    ...
    */
    if (NULL!=pChassis && NULL!=nmu_status)
    {
        ...
        if (utype>1)
        {
            if (nmu_status->sfp3_exist==1)
            {
                if (nmu_status->sfp3_status==1)
                    sfp3=1;
                else
                    sfp2=2;
            }
    ...
    nBytesSent=websWrite(wp, T("<div class=\"ubd1\"><div class=\"un\"><a href='sys/sys_info.asp' target='mf'>NMU</a></div>"));
    if (utype>1)
    {
        led_type='v';
        nBytesSent+=websWrite(wp, T("<div class=\"vleds\">"));
    }
    else
        nBytesSent+=websWrite(wp, T("<div class=\"leds\">"));
    nBytesSent+=websWrite(wp, T("<div class=\"ll\">PWR1</div>"));
    /*nBytesSent+=websWrite(wp, T("<div class=\"led\"><img id=\"pwr1\" src=\"images/%cled_%d.png\" /></div> \
       <div class=\"led\"><img id=\"pwr2\" src=\"images/%cled_%d.png\" /></div> \
       <div class=\"ll\">PWR2</div> \
       <div class=\"ll\">FAN</div> \
       <div class=\"led\"><img id=\"fan\" src=\"images/%cled_%d.png\" /></div> \
       <div class=\"led\"><img id=\"run\" src=\"images/%cled_1.png\" /></div> \
       <div class=\"ll\">RUN</div> \
       <div class=\"ll\">SFP1</div> \
       <div class=\"led\"><img id=\"osc1\" src=\"images/%cled_%d.png\" /></div> \
       <div class=\"led\"><img id=\"osc2\" src=\"images/%cled_%d.png\" /></div> \
       <div class=\"ll\">SFP2</div> \
       <div class=\"clr\"></div> \
      </div>"),led_type,power[0],led_type,power[1],led_type,fan,led_type,led_type,sfp1_led,led_type,sfp2_led);
      */
    ...
    <div class=\"ll\">FAN</div> \
    <div class=\"led\"><img id=\"fan\" src=\"images/%cled_%d.png\" /></div> \
    <div class=\"led\"><img id=\"run\" src=\"images/%cled_%d.png\" /></div>"),led_type,power[0],led_type,power[1],led_type,fan,led_type,run_led);
    if (utype<2)
        nBytesSent+=websWrite(wp, T("<div class=\"ll\">RUN</div>"));
    else
        nBytesSent+=websWrite(wp, T("<div class=\"ll\">SFP3</div>"));
    nBytesSent+=websWrite(wp, T("<div class=\"ll\">SFP1</div> \
    <div class=\"led\"><img id=\"osc1\" src=\"images/%cled_%d.png\" /></div> \
    <div class=\"led\"><img id=\"osc2\" src=\"images/%cled_%d.png\" /></div> \
    <div class=\"ll\">SFP2</div> \
    <div class=\"clr\"></div> \
   </div>"),led_type,sfp1_led,led_type,sfp2_led);
 ...
 <div class=\"pimg\"><img id=\"sfp1\" style=\"width:20px;height:28px;\" src='images/p2_%d.gif'/></div> \
    ...
    <div class=\"pimg\"><img id=\"sfp3\" style=\"width:20px;height:28px;\" src=\"images/p2_%d.gif\"/></div> \
    </div>"),sfp3);
    }
    nBytesSent+=websWrite(wp, T("<div class=\"clr\"></div> \
	...
	<div class=\"pl\">CON</div>"),lan[0],lan[1]);
    //if (utype<2)
    nBytesSent+=websWrite(wp, T("<div class=\"pimg\"><img src='images/p1_2.gif'/></div></div>"));
    if (utype<2)
        nBytesSent+=websWrite(wp, T("<div class=\"clr\"></div> <div class=\"adpter vsb0\"> \
...
int aspNot4UNmuView(webs_t wp,uchar_t utype,uchar_t slot)
{
    char n;
    //uchar_t slot_of_nmu;
    int nBytes=0;
...
int aspNmuView(webs_t wp,uchar_t uclass,uchar_t utype,uchar_t slot)
{
    if (CC_4U==uclass)
        return (asp4UNmuView(wp,utype,slot));
    else
        return (aspNot4UNmuView(wp,utype,slot));
}
...
int showUnitBodyView(webs_t wp,int slot,uchar_t cclass,uchar_t uclass,uchar_t utype)
{
    ...
    case UC_PASSIVE:
        ...
        break;
    case UC_NMU:
        nBytes +=aspNmuView(wp,cclass,utype,slot);
        break;
    }
    return nBytes;
}
...
int aspMakeJscript(int eid, webs_t wp, int argc, char_t **argv,char_t cclass)
{
    ...
    web_unit_t uweb;
    uchar_t slotCount,max_slot_count=16,uclass,utype=0;
    int n,m,nBytes=0;
    uchar_t alarms[17]= {0};
    uchar_t sfp[3]= {3,3,0},lan[2]= {2,2},fan_led=1,sfp_led[3]= {3,3,1},pwr[2]= {3,3};

    cclass=getUnitClass(0,0);//check the type of classis
    ...
    getUnitClassType(0,1,&uclass,&utype);         //get the type of ncp
    //printf("ncp uclass is %d,utype is %d",uclass,utype);
    pnmu=(nmuStatus_t*)getInfoSetValues(0,1,8,1); //get the address of ncp
    if (pnmu!=NULL)
    {
        ...
        if (utype>1)
        {
            if (pnmu->sfp3_exist!=2)
            {
                if (pnmu->sfp3_status!=2)
                    sfp[2]=1;
                else
                    sfp[2]=2;
            }
    ...
    nBytes+=websWrite(wp,T("nmu.slot=%d;\nnmu.un=\"NCP\";nmu.vicount=0;\nnmu.slotCount=1;\nnmu.uclass=%d;\nnmu.utype=%d;\np=new Array(6);\n"),1,uclass,utype);

	//port of ncp
    nBytes+=websWrite(wp,"p.pl=new Array([\"LAN1\",\"LAN2\",\"SFP1\",\"SFP2\",\"SFP3\"]);\np.pimg=new Array([\"epu\",\"epu\",\"sfp2_\",\"sfp2_\",\"sfp2_\"]);\n");
    nBytes+=websWrite(wp,T("p.pstatus=new Array(%d,%d,%d,%d,%d,2);\n"),sfp[0],sfp[1],sfp[2],lan[0],lan[1]);
    nBytes+=websWrite(wp,"p.pclass=\"tm15 imgw30\";p.hi=\"250px\";");

    //led
    nBytes+=websWrite(wp,"l=new Object();\n");
    nBytes+=websWrite(wp,T("l.leds=new Array([0,%d,%d,%d]);\n"),sfp_led[0],sfp_led[1],sfp_led[2]);
    nBytes+=websWrite(wp,"l.txt=new Array([\"PWR\",\"R1\"],[\"R2\",\"R3\"]);\n");

	nBytes+=websWrite(wp,T("nmu.led=l;\nnmu.ports=p;\n"));
    for (n=2; n<max_slot_count; n++)
    {
        if (getUnitViewInfo(0,n,&info)!=FALSE)
        {
            ...
            {
                nBytes+=websWrite(wp,"l=new Object();\n");
                nBytes+=websWrite(wp,T("l.ledtype=%d;\n"),info.leds.led_type);
                nBytes+=websWrite(wp,T("l.leds=new Array([%d,%d,%d,%d],[%d,%d,%d,%d]"),info.leds.leds_status[0][0],info.leds.leds_status[0][1],info.leds.leds_status[0][2],info.leds.leds_status[0][3],info.leds.leds_status[1][0],info.leds.leds_status[1][1],info.leds.leds_status[1][2],info.leds.leds_status[1][3]);
                if (info.leds.row_count==3)
                    nBytes+=websWrite(wp,T(",[%d,%d,%d,%d]"),info.leds.leds_status[2][0],info.leds.leds_status[2][1],info.leds.leds_status[2][2],info.leds.leds_status[2][3]);
                nBytes+=websWrite(wp,");\n");
                //nBytes+=websWrite(wp,T("l.txt=new Array([\"%s\""),info.ports.ports_text[0][0]);
                nBytes+=websWrite(wp,T("\nl.rowCount=%d;l.colCount=%d;\nl.runLed=%d;\nl.runTxt=\"%s\";\n"),info.leds.row_count,info.leds.col_count,info.leds.run_led,info.leds.run_text);
    ...
    nBytes+=websWrite(wp,"showChassis(units,nmu,ps,alm,1);\n</script>\n</td>");
#else
    nBytes+=websWrite(wp,"showChassis(units,nmu,ps,alm,2);\n</script>\n</td>");
#endif
    return nBytes;
}
