...
int asp4UOtuView(webs_t wp, int slot,uchar_t uclass,int otu_type,char show_property)
{
  ...
  uchar_t port[8]={0},led[8]={0},mode=1;
  char ledsLabels[4][3]={""};
  ...
  */
  ...
  if (OTU_TYPE_2Dot5G_SFP_A==otu_type || 3==otu_type || 15==otu_type)
     max_port=8;
  ...
  if (NULL!=pOtuStatus)
  {
   ...
   else
   {
     ...
     led[0]=otu1x->led_status & 0x03;
     tx_enable[0]=otu1x->tx_status & 0x03;
     /*
     led[1]=(otu1x->led_status>>2) & 0x03;
     led[2]=(otu1x->led_status>>4) & 0x03;
     led[3]=(otu1x->led_status>>6) & 0x03;
     led[4]=(otu1x->led_status>>8) & 0x03;
     led[5]=(otu1x->led_status>>10) & 0x03;
     led[6]=(otu1x->led_status>>12) & 0x03;
     led[7]=(otu1x->led_status>>14) & 0x03;
     */
     for (n=1;n<8;n++)
     {
     ...
     for (n=7;n>=0;n--)
     {
       if (bits[n]==1)
       {
         ...
         port[7-n]=0;
       //printf("port #%d=%d[%d:%d];",8-n,bits[n],port[7-n],led[7-n]);
     }
     /*for (n=0;n<8;n++)
     {
      if (bits[n]==1)
...
<div class='vp'><span>%s<img  src='images/led_%d.png' /><img src='images/led_%d.png' /><img src='images/led_%d.png' /><img src='images/led_%d.png' />%s</span></div>\
  ...
  <div class='un'><a href='/goform/unit?cid=0&uid=%d' target='mf'>%s</a></div>\
  <div class='vleds'> \
<div class='vp'><span>PWR<img style='margin-left:2px;' src='images/vled_1.png' /></span> </div>\
<div class='vp'><span>%s<img  src='images/vled_%d.png' /><img src='images/vled_%d.png' /><img src='images/vled_%d.png' /><img src='images/vled_%d.png' />%s</span></div>\
<div class='vp'><span>%s<img  src='images/vled_%d.png' /><img src='images/vled_%d.png' /><img src='images/vled_%d.png' /><img src='images/vled_%d.png' />%s</span></div>\
</div>"),slot,uname,ledsLabels[0],led[0],led[2],led[4],led[6],ledsLabels[2],ledsLabels[1],led[1],led[3],led[5],led[7],ledsLabels[3]);
}
...
static int writeOtuInfo(webs_t wp, int slot,int utype,int group)
{
  ...
  if (2==utype||3==utype ||6==utype ||15==utype)
    port_count=8;
  ...
  else
  {
     ...
     uchar_t led[8]={0};
     uchar_t tx_enable[8];
     uchar_t power_status[8]={0};
     uchar_t ddm_or_los[8]={0};
     uchar_t src_thld[8]={0};
     char *property;
     property=getUnitProperty(0,slot);
     if (property!=NULL)
        nBytes+=websWrite(wp, T("var withCdr=new Array(%d,%d,%d,%d);\n"),property[21],property[22],property[23],property[24]);
     else
        nBytes+=websWrite(wp, T("var withCdr=new Array(0,0,0,0);\n"));
  
     otu1XInfo_t *otu1x=(otu1XInfo_t*)pOtuStatus;
     //status=otu1x->led_status;
     //otu1x->tx_status=otu1x->tx_status>>16;
     bytetobits(otu1x->exist & 0xff,bits);
     //printf("exist=%0x otu1x->tx_status=%0x ps=%0x ddm_or_los=%dsrc_thld=%d\n",otu1x->exist,otu1x->tx_status,otu1x->power_status,otu1x->ddm_or_los,otu1x->src_thld);
     led[0]=otu1x->led_status & 0x03;
     tx_enable[0]=otu1x->tx_status & 0x03;
     power_status[0]=otu1x->power_status & 0x01;
     ddm_or_los[0]=otu1x->ddm_or_los & 0x03;
     src_thld[0]=otu1x->src_thld & 0x01;
     for (n=1;n<8;n++)
     {
         led[n]=(otu1x->led_status>>n*2) & 0x03;
         tx_enable[n]=((otu1x->tx_status>>n*2) & 0x03);
         power_status[n]=((otu1x->power_status>>n) & 0x1);
         ddm_or_los[n]=(otu1x->ddm_or_los>>n*2) & 0x03;
         src_thld[n]=(otu1x->src_thld>>n) & 0x1;
     }
     for (n=1;n<=port_count;n++)
     {
        /*
        ...
        */
        ...
        if (bits[8-n]==1)
        {
         if (group<1)
         {
          //printf("speed=%d\n",pPortProperty->current_speed);
          nBytes+=websWrite(wp, T("p[%d]= new Object();\n"),count);
          nBytes+=websWrite(wp, T("p[%d].pn=%d;p[%d].d=%d;p[%d].wl=%d;p[%d].speed=%d;p[%d].ws=%d;p[%d].ts=%d;\n"),count,n,count,pPortProperty->distance,count,pPortProperty->wave_lenght,count,pPortProperty->current_speed,count,led[n-1],count,tx_enable[n-1]);
          nBytes+=websWrite(wp, T("p[%d].ps=%d;p[%d].ledmode=%d;p[%d].almsrc=%d;"),count,power_status[n-1],count,ddm_or_los[n-1],count,src_thld[n-1]);
          if (NULL!=pOtuPortFEC)
            nBytes+=websWrite(wp, T("p[%d].lb=%d;"),count,pOtuPortFEC->loopback);
          count++;
         }
         else
         {
            if(n==getSelectedPort())
             {
             nBytes=websWrite(wp, T("p.pn=%d;p.d=%d;p.wl=%d;p.speed=%d;p.ws=%d;p.ts=%d;p.ps=%d;p.ledmode=%d;p.almsrc=%d;\n"),n,pPortProperty->distance,pPortProperty->wave_lenght,pPortProperty->current_speed,pOtuStatus->portStatus[n-1].port_status,tx_enable[n-1],power_status[n-1],ddm_or_los[n-1],src_thld[n-1]);
             if (NULL!=pOtuPortFEC)
               nBytes+=websWrite(wp, T("p.lb=%d;"),pOtuPortFEC->loopback);
             return nBytes;
             }
