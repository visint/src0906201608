...
static void showUnitInfo(uchar_t chassis,uchar_t slot)
{
   ...
   infoset.infoset_id=CHASSIS_INFOSET_ID;
   infoset.index=0;
   if (getStatusInfoSet(0,0,&infoset))
     printf("getStatusInfoSet ok\n");
   else
     printf("getStatusInfoSet fail\n");
   ...
   return ;
    ...
    for (n=0;n<count;n++)
    {
      ...
      {
        printf("%s:%s(item id=%d value=%d)\n",iset_ex.item_name[m],iset_ex.status_name[m],m+1,pis->item_values[m]);
      }
   ...
   {
     otuInfo_t *info;
     info=getOtuInfo(chassis,slot,1);
    // if (info!=NULL)
